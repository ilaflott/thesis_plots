...
void  makeSMPInclJetXsec_OnlyNLO (std::string outdir, TFile* fout){
  ...
  for(int i=0; i<netabins; i++){
    ...
    mcspectra[i]=  (TH1D*)make_fNLOSpectra( (std::string) "CT14nnlo", (std::string) "murmufHTp_v5", (std::string) "1",
					    (int) i, (int) 0,
  ...
  mcleg->AddEntry((TObject*)0, "CT14 NNLO", "");
...
void  makeSMPInclJetXsec_NLOsyst_targPDF_ratios (std::string outdir, TFile* fout, std::string targPDF, std::string scalechoice, std::string order){
  ...
  for(int i=0; i<netabins; i++){
    ...
    //setRatioHistLabels((TH1D*)ratios[i], targPDF_nous+" "+order+" Relative Unc.");
    //setRatioHistLabels((TH1D*)ratios[i], targPDF_nous+" "+order+" Unc.");
    setRatioHistLabels((TH1D*)ratios[i], order+" Unc.");
  ...
  for(int i=0; i<netabins; i++){
    ...
    if(i==1){
      TLegend* leg=makeLegend(0.61, 0.60, 0.98, 0.90);
...
void  makeSMPInclJetXsec_NPCs_onePadAllEta (std::string outdir, TFile* fout, std::string NPC_str){
  ...
  TF1* NPCfit[netabins]={};
  TH1D* NPCfithist[netabins]={};//to make it easier to draw stuf
  ...
  //std::cout<<"NPCfithist[0]->GetNbinsX()="<<NPCfithist[0]->GetNbinsX()<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinLowEdge(1)="<<NPCfithist[0]->GetBinLowEdge(1)<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinLowEdge(nbins)+width="<<NPCfithist[0]->GetBinLowEdge(NPCfithist[0]->GetNbinsX()) + NPCfithist[0]->GetBinWidth(NPCfithist[0]->GetNbinsX())<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinWidth(1)="<<NPCfithist[0]->GetBinWidth(1)<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinWidth(10)="<<NPCfithist[0]->GetBinWidth(10)<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinWidth(20)="<<NPCfithist[0]->GetBinWidth(20)<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinWidth(30)="<<NPCfithist[0]->GetBinWidth(30)<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinContent(1)="<<NPCfithist[0]->GetBinContent(1)<<std::endl;
  //std::cout<<"NPCfithist[0]->GetBinContent(nbins)="<<NPCfithist[0]->GetBinContent(NPCfithist[0]->GetNbinsX())<<std::endl;
  //return;
  
  int nbins= NPCfithist[0]->GetNbinsX();
  double minpt= NPCfithist[0]->GetBinLowEdge(1);
  double maxpt= NPCfithist[0]->GetBinLowEdge(nbins)+NPCfithist[0]->GetBinWidth(nbins);

  TH1D* fordrawingonlyhist=new TH1D( ("forNPCdrawingOnly_"+NPC_str).c_str(), "", nbins,minpt,maxpt);
  fordrawingonlyhist->SetMaximum(maxy*1.02);
  fordrawingonlyhist->SetMinimum(miny*.99);
  fordrawingonlyhist->SetTitle("");  
  fordrawingonlyhist->GetXaxis()->SetTitle("Jet p_{T} [GeV]");
  fordrawingonlyhist->GetXaxis()->SetNoExponent(true);
  fordrawingonlyhist->GetXaxis()->SetMoreLogLabels(true);  
  fordrawingonlyhist->GetYaxis()->SetTitle("Correction Factor");
  fordrawingonlyhist->GetYaxis()->CenterTitle(true);
  //fordrawingonlyhist->SetLineColorAlpha(kRed, 0.00);
  //fordrawingonlyhist->SetMarkerColorAlpha(kRed, 0.00);
  ...
  //NPCfithist[0]->SetLineColorAlpha(kRed, 0.00);
  //NPCfithist[0]->SetMarkerColorAlpha(kRed, 0.00);
...
void  makeSMPInclJetXsec_NPCs_onePadOneEta (std::string outdir, TFile* fout, std::string NPC_str){
  ...
  for(int i=0; i<netabins; i++){
    ...
    NPCdata_forDraw[i]->SetMinimum(0.84);      NPCdata_forDraw[i]->SetMaximum(1.09);                
    NPCdata_forDraw[i]->SetAxisRange(xmin[i],xmax[i], "X");
    
    TLine* one     =makeTLine(xmin[i], 1. , xmax[i], 1.);          
    
    TPaveText* desc=makePaveText( 0.5, .82, 0.7, .9);
    desc->AddText(const_ybin_strs[i].c_str());
