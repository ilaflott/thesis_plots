...
TCanvas* makeSMPTH2Canvas(           std::string);
...
double* setBinning_etabin(int , int* , bool  );
void makeTotRelSystUncRatio(std::string ,TH1*, std::vector<TH1*> , TH1*, bool);
void makeTotSystUncRatio(std::string ,TH1*, TH1*, std::vector<TH1*> , TH1*, bool);
...
std::string makeProperPDFname(std::string);
std::string makeProperPDFname2(std::string);
...
TCanvas* makeSMPRatioCanvas( std::string name="defname"){
  ...
  for(int i=1; i<=npads; i++){
    ...
    c->cd(i)->SetRightMargin(0.011);
    c->cd(i)->SetTopMargin(0.07);
  }
...
TCanvas* makeSMPTH2Canvas( std::string name="defname"){
  bool funcDebug=true;
  if(funcDebug)std::cout<<"in makeSMPTH2Canvas"<<std::endl;
...
const float   ratpavex1=.05,   ratpavey1=.94,   ratpavex2=.95,   ratpavey2=1.0;
TPaveText* makePrelimPaveTextTitle(){
...
TPaveText* makePrelimPaveTextTitleTH2(){
  ...
  title->SetFillStyle(0);
  title->SetBorderSize(0.);
  title->AddText(prelimth2pavetitle.c_str());
  return (TPaveText*)title;
}
...
TPaveText* makeSimPaveTextTitleTH2(){
  ...
  title->SetFillStyle(0);
  title->SetBorderSize(0.);
  title->AddText(simth2pavetitle.c_str());
  return (TPaveText*)title;
}
...
////for approval settings / NNPDFs figure
const float rat_ytitlesize=0.055, rat_xtitlesize=0.046;
const float rat_ytitleoffset=.80, rat_xtitleoffset=1.03;
const float rat_ylabelsize=0.06,  rat_xlabelsize=0.046;
...
double* setBinning_etabin(int etabinint=0, int* nbins=NULL , bool forJohn=false){
  bool funcDebug=true;
  ...
  if(!forJohn){
    ...
    *(nbins)=(int)(merged_SMP_ptbins[etabinint].size()-1);
    ...
    //binarray=(double*)(merged_SMP_ptbins[etabinint].data());
    //*(nbins)=(int)(merged_SMP_ptbins[etabinint].size()-1);
    binarray=(double*)(johnsbins_semifin[etabinint].data());
    *(nbins)=(int)(johnsbins_semifin[etabinint].size()-1);    
  }
  
  if(funcDebug)std::cout<<std::endl<<"setBinning_etabin (double*) done, exiting."<<std::endl<<std::endl;
  return binarray;
  
}


void makeTotRelSystUncRatio(std::string dir="up",TH1* statunc=NULL, std::vector<TH1*> systhists={0}, TH1* targ=NULL, bool addlumisys=false){
  bool funcDebug=true;
  ...
  for(int j=1; j<=nbins; j++){
	...
	if(addlumisys)targ->AddBinContent(j,pow(lumiunc,2));
      }
      else{
...
void makeTotSystUncRatio(std::string dir="up", TH1* ratio=NULL, TH1* statunc=NULL, std::vector<TH1*> systhists={0}, TH1* targ=NULL, bool addlumisys=false){
  bool funcDebug=false;
  ...
  for(int j=1; j<=nbins; j++){
    ...
    for(int i=0; i<= nsysthists; i++){
      if(i==nsysthists){
	targ->AddBinContent(j,pow(statunc->GetBinError(j),2));
	if(addlumisys)	targ->AddBinContent(j,pow(ratio->GetBinContent(j)*lumiunc,2));
      }
      else{
...
TH1D* get_fNLOSpectra_6PUnc(  std::string targPDF, std::string scalechoice, std::string orderint, 
			      int etabin, std::string upORdown,
			      bool rebin, std::vector<double> bins,
			      bool applyNPCs ){
  bool funcDebug=true;
  if(funcDebug)
    std::cout<<"running get_fNLOSpectra_6PUnc!"<<std::endl;   
  
  std::string thyfilename=FNLO_DIR+"R04/fout_"+targPDF+"_"+scalechoice+".root";
  if(funcDebug)std::cout<<"grabbing theory hist(s) from thyfilename =="<<thyfilename<<std::endl;
  
  int spectratype=-1;
  if(upORdown=="up")spectratype=8;
  else if(upORdown=="down")spectratype=9;  
  if(spectratype<0)return NULL;

  TFile* thyfile=TFile::Open(thyfilename.c_str(),"READ");
  std::string hname="h"+orderint+"100"+std::to_string(etabin+1)+"0"+std::to_string(spectratype)+"_spectra";
  
  std::string newhname=targPDF;
  if(spectratype==1)newhname+="_PDFsysdown";  
  else if(spectratype==2)newhname+="_PDFsysup";  
  newhname+="_ybin"+std::to_string(etabin);
  
  TH1D* mcspectra_6Punc=(TH1D*)(
				(
				 (TH1D*)thyfile->Get(hname.c_str()) 
				 )->Clone( newhname.c_str()) 
				);  
  ...
  mcspectra_6Punc->Scale(1./1000.);
  multiplyBinWidth(mcspectra_6Punc);    
  if(applyNPCs) applyNPCorrs(mcspectra_6Punc,etabin, "");
  if(rebin)
    mcspectra_6Punc=(TH1D*)mcspectra_6Punc->TH1::Rebin( (bins.size()-1), ((std::string)mcspectra_6Punc->GetName()+"_rebin").c_str() , (double*)bins.data());
  divideBinWidth(mcspectra_6Punc);    
  if(funcDebug)mcspectra_6Punc->Print("base");
  
  //so at this point we have a spectra w/ NPs shifted up/down by the scale unc. Now i need to turn that back into a fractional unc. Now i need the usual PDF to do that. 
  TH1D* mcspectra = make_fNLOSpectra(  targPDF, scalechoice, orderint, 
				       etabin , 0 ,
				       rebin, bins, 
				       applyNPCs , "" );
  int nxbins=mcspectra_6Punc->GetNbinsX();
  for(int i=1; i<=nxbins; i++){
    std::cout<<"mcspectra             ->GetBinContent("<<i<<")="<<mcspectra->GetBinContent(i)<<std::endl;
    std::cout<<"BEFORE mcspectra_6Punc->GetBinContent("<<i<<")="<<mcspectra_6Punc->GetBinContent(i)<<std::endl;
    mcspectra_6Punc->SetBinContent( i, (
					(mcspectra_6Punc->GetBinContent(i) - mcspectra->GetBinContent(i) )/ mcspectra->GetBinContent(i) // ( delta(t) ) / t
					) );
    std::cout<<"AFTER  mcspectra_6Punc->GetBinContent("<<i<<")="<<mcspectra_6Punc->GetBinContent(i)<<std::endl;
  }
  mcspectra->Delete();
  //now each bin represents the fractional error (unitless, NOT % like in the fNLO file).   
  return (TH1D*)mcspectra_6Punc;
}
...
std::string makeProperPDFname2(std::string targstr){
  bool funcDebug=false;
  if(funcDebug)std::cout<<"running makeProperPDFname("<<targstr<<")"<<std::endl;
  std::string outstr="";
  if(targstr.find("NNPDF")!=std::string::npos){//NNPDF names have underscores + spaces in them
    if(funcDebug)std::cout<<"NNPDF name (before) = "<<targstr<<std::endl;
    outstr=replace_underscores(targstr); //e.g.: NNPDF31_nnlo_as_0122 --> NNPDF31 nnlo as 0122
    if(funcDebug)std::cout<<"... removed underscores = "<<outstr<<std::endl;
    outstr.replace(outstr.find("nnlo "), 12, "");// --> NNPDF31
    if(funcDebug)std::cout<<"... removed 'nnlo as 0XXX' = "<<outstr<<std::endl;    
    if(     outstr.find("31")!=std::string::npos)    outstr.replace(outstr.find("31"),2,"3.1");
    else if(outstr.find("30")!=std::string::npos)    outstr.replace(outstr.find("30"),2,"3.0");
    else if(outstr.find("23")!=std::string::npos)    outstr.replace(outstr.find("23"),2,"2.3");
    if(funcDebug)std::cout<<"... added decimal to '31' = "<<outstr<<std::endl;    
  }
  else if(targstr.find("CT")!=std::string::npos){//it's a CT14 name and we can do less work.
    if(funcDebug)std::cout<<"CT PDF name (before) = "<<targstr<<std::endl;
    //outstr=targstr.replace( targstr.find("nnlo"),4,"");
    outstr=targstr.substr(0,4);
    if(funcDebug)std::cout<<"CT PDF name (after) = "<<outstr<<std::endl;        
  }
  else if(targstr.find("ABMP16")==std::string::npos)
    outstr="ABMP16 n_{f}=5";
  else if(targstr.find("MSHT20")==std::string::npos)
    outstr="MSHT20";
  else{//it's a CT14 name and we can do less work.
    if(funcDebug)std::cout<<"CT14 PDF name (before) = "<<targstr<<std::endl;
    outstr=targstr.replace( targstr.find("nnlo"),4,"");
    if(funcDebug)std::cout<<"CT14 PDF name (after) = "<<outstr<<std::endl;    
...
std::string makeProperPDFname(std::string targstr){
  bool funcDebug=false;
  if(funcDebug)std::cout<<"running makeProperPDFname("<<targstr<<")"<<std::endl;
  std::string outstr="";
  if(targstr.find("NNPDF")!=std::string::npos){//NNPDF names have underscores + spaces in them
    ...
    outstr=replace_underscores(targstr); //e.g.: NNPDF31_nnlo_as_0122 --> NNPDF31 nnlo as 0122
    if(funcDebug)std::cout<<"... removed underscores = "<<outstr<<std::endl;
    outstr.replace(outstr.find("nnlo "), 5, "");// --> NNPDF31 as 0122
    if(funcDebug)std::cout<<"... removed 'nnlo ' = "<<outstr<<std::endl;    
    outstr.replace(outstr.find("as 0"),4, "#alpha_{S}(M_{Z})=0.");
    if(funcDebug)std::cout<<"... removed 'as 0' = "<<outstr<<std::endl;    
    if(     outstr.find("31")!=std::string::npos)    outstr.replace(outstr.find("31"),2,"3.1");
    else if(outstr.find("30")!=std::string::npos)    outstr.replace(outstr.find("30"),2,"3.0");
    else if(outstr.find("23")!=std::string::npos)    outstr.replace(outstr.find("23"),2,"2.3");
    if(funcDebug)std::cout<<"... added decimal to '31' = "<<outstr<<std::endl;    
