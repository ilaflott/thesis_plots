...
void printCanvases(int jobnum=-1){
  ...
  switch (jobnum)
    ...
    case 8:  std::cout<<"you chose JETTRIGQA  "<<std::endl    ; file_array=(std::string*)  JETTRIGQA_file_array       ;shortdir_array=(std::string*)         JETTRIGQA_shortdir_array;  targdir=JETTRIGQA         ; Nfiles=  JETTRIGQA_Nfiles       ;break;
      //case 9:  std::cout<<"you chose JERDIR2_MC  "<<std::endl    ; file_array=(std::string*)  JERDIR2_MC_file_array       ;shortdir_array=(std::string*)         JERDIR_MC_shortdir_array;  targdir=JERDIR_MC         ; Nfiles=  JERDIR_MC_Nfiles       ;break;
  ...
  for(int i=0; i<Nfiles;i++){  
    ...
    while ( (key = (TKey*)next()) ) {//loop over all objects in the root file
      ...
      //(jobnum==9)//for pat/john DSCB fits and my gauss fits on same panels etc. //does not work. do not do.
      //if( ((std::string)key->GetName()).find("JERfitCanv_3x3")==std::string::npos ){
      //  std::cout<<"jobnum==9, but key->GetName()="<<key->GetName()<<" doesn't contain 'JERfitCanv_3x3', skip! \n";
      //  continue; 
      //}
